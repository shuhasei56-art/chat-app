...
// === News fetch (same-origin) ===
// Cloudflare Pages Functions の /api/news が Yahoo RSS/記事を代理取得します。
// フロントは必ず同一オリジンのAPIを叩くことで CORS / 403 / 422 を回避します。
const fetchTextViaNewsApi = async (url, { timeoutMs = 8000 } = {}) => {
...
const NewsView = ({ showNotification }) => {
  ...
  const loadYahooRss = useCallback(async () => {
    ...
    try {
      ...
      const text = await fetchTextViaNewsApi(rssUrl, { timeoutMs: 8000 });
  ...
  const openArticle = useCallback(async (item) => {
    ...
    setArticleText("");
    try {
      const htmlText = await fetchTextViaNewsApi(item.link, { timeoutMs: 10000 });
