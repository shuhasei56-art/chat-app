...
import {
  ...
  onAuthStateChanged,
  setPersistence,
  browserSessionPersistence, // 変更点: セッションごとの保存（タブを閉じるとログアウト）
...
import {
  ...
  Plus, Video, Heart, MessageCircle, Camera as CameraIcon, Maximize, Upload, Copy,
  Gift, Cake, Loader2, Mic, MicOff, VideoOff, Phone, PhoneOff, Coins, AtSign, KeyRound, User, PartyPopper
} from 'lucide-react';
...
// 2. Birthday Card Modal (NEW)
const BirthdayCardModal = ({ friend, onClose, onSendGift }) => {
  return (
    <div className="fixed inset-0 z-[700] bg-black/80 flex items-center justify-center p-6 backdrop-blur-md animate-in fade-in zoom-in">
      <div className="bg-gradient-to-br from-pink-100 to-yellow-100 w-full max-w-sm rounded-[40px] p-8 text-center shadow-2xl relative overflow-hidden border-4 border-white">
        {/* Decorative Circles */}
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
           <div className="absolute top-[-20px] left-[-20px] w-24 h-24 bg-pink-300 rounded-full opacity-50 blur-xl"></div>
           <div className="absolute bottom-[-20px] right-[-20px] w-32 h-32 bg-yellow-300 rounded-full opacity-50 blur-xl"></div>
        </div>
        
        <div className="relative z-10">
          <div className="mx-auto w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-lg mb-4 animate-bounce">
            <Cake className="w-12 h-12 text-pink-500" />
          </div>
          <h2 className="text-3xl font-black text-pink-500 mb-1 drop-shadow-sm font-script">Happy Birthday!</h2>
          <p className="text-gray-600 text-sm font-bold mb-6">今日は {friend.name} さんの<br/>お誕生日です！🎉</p>
          
          <img src={friend.avatar} className="w-32 h-32 rounded-full border-[6px] border-white shadow-xl mx-auto mb-8 object-cover" />
          
          <button onClick={onSendGift} className="w-full bg-gradient-to-r from-pink-500 to-orange-400 text-white font-bold py-4 rounded-2xl shadow-lg hover:scale-[1.02] transition-transform flex items-center justify-center gap-2 mb-3">
            <Gift className="w-5 h-5" /> お祝いを送る
          </button>
          <button onClick={onClose} className="text-gray-400 text-xs font-bold hover:text-gray-600">閉じる</button>
        </div>
      </div>
    </div>
  );
...
// 4. Friend Profile Modal (Updated with Birthday Trigger)
const FriendProfileModal = ({ friend, onClose, onStartChat, onTransfer, onOpenBirthdayCard }) => (
    ...
    <div className="bg-white w-full max-w-sm rounded-[40px] overflow-hidden shadow-2xl relative flex flex-col items-center pb-8">
      ...
      <div className="-mt-16 mb-4 relative">
        ...
        {isTodayBirthday(friend.birthday) && (
             <button onClick={() => onOpenBirthdayCard(friend)} className="absolute bottom-0 right-0 bg-pink-500 text-white p-2 rounded-full shadow-lg hover:scale-110 transition-transform animate-bounce">
      ...
      <h2 className="text-2xl font-bold mb-1 flex items-center gap-2">
        {friend.name}
        {isTodayBirthday(friend.birthday) && <Cake className="w-5 h-5 text-pink-500" />}
      </h2>
...
const AuthView = ({ onLogin, showNotification }) => {
  ...
  const [birthday, setBirthday] = useState(""); // 誕生日入力追加
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {!isLoginMode && (
            <>
              ...
              <div className="space-y-1"><label className="text-[10px] font-bold text-gray-400 ml-2">誕生日</label><div className="bg-gray-50 rounded-2xl px-4 py-3 flex items-center gap-2 border"><Cake className="w-4 h-4 text-gray-400" /><input type="date" className="bg-transparent w-full outline-none text-sm font-bold" value={birthday} onChange={(e) => setBirthday(e.target.value)} /></div></div>
            </>
          )}
  ...
  const renderContent = (text) => {
    ...
    return text.split(regex).map((part, i) => {
      ...
      if (part.startsWith('@')) {
        ...
        if (u) return <span key={i} className="text-blue-500 font-bold cursor-pointer hover:underline bg-blue-50 px-1 rounded" onClick={(e) => { e.stopPropagation(); onShowProfile(u); }}>{part}</span>;
      }
      return part;
    });
  };
    ...
    <div className={`flex ${isMe ? "justify-end" : "justify-start"} gap-2 relative group mb-4`}>
      {!isMe && (
        <div className="relative mt-1 cursor-pointer" onClick={(e) => { e.stopPropagation(); onShowProfile(sender); }}>
          ...
          {isTodayBirthday(sender?.birthday) && <span className="absolute -top-1 -right-1 text-[10px]">🎂</span>}
        </div>
      )}
...
const ChatRoomView = ({ user, profile, allUsers, chats, activeChatId, setActiveChatId, setView, db, appId, showNotification, addFriendById, startVideoCall }) => {
  ...
  const [coinModalTarget, setCoinModalTarget] = useState(null), [birthdayModalTarget, setBirthdayModalTarget] = useState(null);
  ...
  return (
    <div className="flex flex-col h-full bg-[#8fb2c9] relative">
      ...
      {viewProfile && <FriendProfileModal friend={viewProfile} onClose={() => setViewProfile(null)} onStartChat={(uid) => { setViewProfile(null); }} onTransfer={() => { setCoinModalTarget(viewProfile); setViewProfile(null); }} onOpenBirthdayCard={(f) => { setBirthdayModalTarget(f); setViewProfile(null); }} />}
      ...
      {birthdayModalTarget && <BirthdayCardModal friend={birthdayModalTarget} onClose={() => setBirthdayModalTarget(null)} onSendGift={() => { setCoinModalTarget(birthdayModalTarget); setBirthdayModalTarget(null); }} />}
    </div>
  );
};

const HomeView = ({ user, profile, allUsers, chats, setView, setActiveChatId, setSearchModalOpen, startChatWithUser, showNotification }) => {
  const [tab, setTab] = useState("chats"), [selectedFriend, setSelectedFriend] = useState(null), [coinModalTarget, setCoinModalTarget] = useState(null), [birthdayModalTarget, setBirthdayModalTarget] = useState(null);
  ...
  return (
    <div className="flex flex-col h-full bg-white border-x border-gray-200 max-w-md mx-auto">
        ...
        {tab === "friends" && friends.map(f => (
          <div key={f.uid} className="p-4 flex items-center gap-4 hover:bg-gray-50 cursor-pointer" onClick={() => setSelectedFriend(f)}>
            <div className="relative">
              ...
              {isTodayBirthday(f.birthday) && <span className="absolute -top-1 -right-1 text-sm">🎂</span>}
            </div>
            <div className="flex-1 font-bold text-sm">{f.name}</div>
          </div>
        ))}
      ...
      {selectedFriend && <FriendProfileModal friend={selectedFriend} onClose={() => setSelectedFriend(null)} onStartChat={startChatWithUser} onTransfer={() => { setCoinModalTarget(selectedFriend); setSelectedFriend(null); }} onOpenBirthdayCard={(f) => { setBirthdayModalTarget(f); setSelectedFriend(null); }} />}
      {coinModalTarget && <CoinTransferModal onClose={() => setCoinModalTarget(null)} myWallet={profile.wallet} myUid={user.uid} targetUid={coinModalTarget.uid} targetName={coinModalTarget.name} showNotification={showNotification} />}
      {birthdayModalTarget && <BirthdayCardModal friend={birthdayModalTarget} onClose={() => setBirthdayModalTarget(null)} onSendGift={() => { setCoinModalTarget(birthdayModalTarget); setBirthdayModalTarget(null); }} />}
    </div>
  );
        ...
        <div className="px-8 -mt-12 flex flex-col items-center gap-6">
          ...
          <div className="w-full space-y-4">
            <div><label className="text-xs text-gray-400">名前</label><input className="w-full border-b py-2 focus:outline-none" value={edit.name || ""} onChange={(e) => setEdit({ ...edit, name: e.target.value })} /></div>
             <div><label className="text-xs text-gray-400">誕生日</label><input type="date" className="w-full border-b py-2 focus:outline-none" value={edit.birthday || ""} onChange={(e) => setEdit({ ...edit, birthday: e.target.value })} /></div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 【重要】ブラウザ（タブ）を閉じるとログイン情報が消える設定に変更
  ...
  useEffect(() => {
    ...
    onSnapshot(query(collection(db, "artifacts", appId, "public", "data", "chats"), where("participants", "array-contains", user.uid)), (snapshot) => {
      ...
      if (incoming) setActiveCall({ chatId: incoming.id, callData: incoming.callStatus, isIncoming: true, isVideo: true });
    });
            ...
            activeCall.isIncoming ? (
              <div className="fixed inset-0 z-[1000] bg-gray-900 flex flex-col items-center justify-center p-6 animate-in fade-in">
                ...
                <div className="flex gap-8">
                  <button onClick={async () => { await updateDoc(doc(db, "artifacts", appId, "public", "data", "chats", activeCall.chatId), { callStatus: deleteField() }); setActiveCall(null); }} className="p-6 bg-red-500 rounded-full shadow-lg text-white"><PhoneOff className="w-8 h-8"/></button>
                  <button onClick={() => setActiveCall({ ...activeCall, isIncoming: false })} className="p-6 bg-green-500 rounded-full shadow-lg text-white animate-bounce"><Video className="w-8 h-8"/></button>
                </div>
              </div>
            ) : (
