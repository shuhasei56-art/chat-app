...
const HomeView = ({ user, profile, allUsers, chats, setView, setActiveChatId, setSearchModalOpen, startChatWithUser, showNotification }) => {
  ...
  return (
    <div
      ...
      onClick={() => { setOpenChatMenuId(null); setOpenFriendMenuId(null); setMenuOpen(false); }}
    >
      <div className="p-4 border-b flex justify-between items-center bg-white shrink-0">
        ...
        <div className="flex gap-4 items-center" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="relative">
            ...
            {menuOpen && (
                <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-xl border z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                    <div className="p-2 space-y-1">
                        <button onClick={() => { setView('profile'); setMenuOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-gray-50 rounded-xl flex items-center gap-3 text-sm font-bold text-gray-700 transition-colors">
                            <User className="w-4 h-4 text-gray-500"/> プロフィール設定
                        </button>
                        <button onClick={() => { setSearchModalOpen(true); setMenuOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-gray-50 rounded-xl flex items-center gap-3 text-sm font-bold text-gray-700 transition-colors">
                            <Search className="w-4 h-4 text-gray-500"/> ユーザー検索
                        </button>
                        <button onClick={() => { setView('qr'); setMenuOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-gray-50 rounded-xl flex items-center gap-3 text-sm font-bold text-gray-700 transition-colors">
                            <UserPlus className="w-4 h-4 text-gray-500"/> QRコード
                        </button>
                        <button onClick={() => { setView('group-create'); setMenuOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-gray-50 rounded-xl flex items-center gap-3 text-sm font-bold text-gray-700 transition-colors">
                            <Users className="w-4 h-4 text-gray-500"/> グループ作成
                        </button>
                        <div className="h-px bg-gray-100 my-1"></div>
                        <button onClick={() => { setView('sticker-store'); setMenuOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-gray-50 rounded-xl flex items-center gap-3 text-sm font-bold text-gray-700 transition-colors">
                            <Store className="w-4 h-4 text-orange-500"/> スタンプショップ
                        </button>
                        <button onClick={() => { setView('birthday-cards'); setMenuOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-gray-50 rounded-xl flex items-center gap-3 text-sm font-bold text-gray-700 transition-colors">
                            <Gift className="w-4 h-4 text-pink-500"/> カードBOX
                        </button>
                    </div>
                </div>
            )}
